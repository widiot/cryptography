...
public class DESECB {
	...
	public static void main(String[] args) {
		String plaintext = "abcdefgh" + "ijklmnop";
		String key = "aaaaaaaa";

		DESECB des = new DESECB();
	...
	public StringBuffer[] getSubKey(String key) {
		...
		// System.out.println("密钥" + keyBinary);
		...
		// System.out.println("PC1置换" + substituteKey);
		...
		// System.out.println("分成左边" + C0 + " " + "右边" + D0);
		...
		for (int i = 0; i < 16; ++i) {
			...
			// System.out.println(i + "移位" + "左边" + C0 + " " + "右边" + D0);

			// 把左右两块合并
			StringBuffer C0D0 = new StringBuffer(C0.toString() + D0.toString());

			// System.out.println(i + "合并密钥" + C0D0);
			...
			// System.out.println(i + "子密钥为" + C0D0Temp);
	...
	public String encrypt(String plaintext, String key, String type) {
		...
		// 2.1 把明文转换成二进制字符串
		int subPlaintext = plaintext.length() / 8;
		StringBuffer[] plaintextBinary = new StringBuffer[subPlaintext]; // 存储明文二进制
		for (int i = 0; i < subPlaintext; ++i) {
			plaintextBinary[i] = new StringBuffer();
			for (int j = 0; j < 8; ++j) {
				StringBuffer mSubPlaintextTemp = new StringBuffer(Integer.toBinaryString(plaintext.charAt(i * 8 + j)));
				while (mSubPlaintextTemp.length() < 8) {
					mSubPlaintextTemp.insert(0, 0);
				}
				plaintextBinary[i].append(mSubPlaintextTemp);
			}
		}
		// System.out.println("明文" + plaintextBinary);

		for (int loop = 0; loop < subPlaintext; ++loop) {
			// 2.2 通过IP置换明文
			StringBuffer substitutePlaintext = new StringBuffer(); // 存储置换后的明文
			for (int i = 0; i < 64; ++i) {
				substitutePlaintext.append(plaintextBinary[loop].charAt(IP_Table[i] - 1));
			}

			// System.out.println("IP" + substitutePlaintext);
			...
			for (int i = 0; i < 16; ++i) {
				...
				// System.out.println("密钥" + subKey[i] + "\n" + "结果" + mRTemp +
				// "\n");
